...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        if args.add_bias:
            self.wq_bias, self.wk_bias, self.wv_bias = [
                nn.Parameter(torch.zeros([self.n_local_heads * self.head_dim])) 
                for _ in range(3)
            ]
        ...
        if args.add_scale:
            self.wq_scale, self.wk_scale, self.wv_scale = [
                nn.Parameter(torch.ones([args.dim]))
                for _ in range(3)
            ]
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None):
        ...
        if adapter is not None:
            output += self.gate[:, self.head_start: self.head_end].tanh().half() * self._forward_scaled_dot_product_attention(xq, adapter_k, adapter_v)
...
class FeedForward(nn.Module):
    def __init__(
        ...
        if add_bias:
            self.w1_bias, self.w3_bias = [
                nn.Parameter(torch.zeros([hidden_dim // mp_size]))
                for _ in range(2)
            ]
    ...
    def forward(self, x):
        return forward_linear_with_scale_and_bias(
            ...
            self.w2, self.w2_scale, self.w2_bias
            )
...
class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.feed_forward = FeedForward(
            ...
            add_bias=args.add_bias, add_scale=args.add_scale,
        )
...
class Transformer(nn.Module):
    ...
    def forward_inference(self, tokens: torch.Tensor, start_pos: int):
        ...
        for i, layer in enumerate(self.layers):
            ...
            h = layer(h, start_pos, freqs_cis, mask,
                adapter[adapter_index].half() if adapter_index >= 0 else None)
